...
def mask_test_edges(adj):
    ...
    test_edges_false = []
    test_edges_false_set = set(map(tuple, test_edges_false))
    ...
    while len(test_edges_false) < len(test_edges):
        ...
        if ismember([idx_i, idx_j], edges_all):
            continue
        if (idx_j, idx_i) in test_edges_false_set or (idx_i, idx_j) in test_edges_false_set:
            continue
        test_edges_false.append([idx_i, idx_j])
        test_edges_false_set.add((idx_i, idx_j))

    # Generate negative samples for the validation set
    val_edges_false = []

    val_edges_false_set = set(map(tuple, val_edges_false))
    while len(val_edges_false) < len(val_edges):
        ...
        if ismember([idx_j, idx_i], val_edges):
            continue
        if (idx_j, idx_i) in val_edges_false_set or (idx_i, idx_j) in val_edges_false_set:
            continue
        val_edges_false.append([idx_i, idx_j])
        val_edges_false_set.add((idx_i, idx_j))

    # Ensure negative samples are not in the original edge set
